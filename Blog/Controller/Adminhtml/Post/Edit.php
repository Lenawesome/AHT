...
class Edit extends \AHT\Blog\Controller\Adminhtml\Post
{
    ...
    {
        $id = $this->getRequest()->getParam('id');//lấy ID trên thanh URL sau đó kiểm tra nó có tồn tại không bằng cách load($id)
        $model = $this->_postFactory->create();

        if ($id) {
            $model->load($id);
            if (!$model->getId()) { // kiểm tra sự tồn tại của ID trên thanh URL trong trường hợp nhập thẳng trên đó
